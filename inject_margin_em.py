...
n = len(MG["d"])
last = MG["rzrqye"][-1]
p5 = MG["rzrqye"][max(0, n - 6)]
p20 = MG["rzrqye"][max(0, n - 21)]
chg5 = (last / p5 - 1) * 100
chg20 = (last / p20 - 1) * 100
...
print(
    "融资买入额(亿)",
    MG["rzmre"][-1],
    "| 占两市成交(按2.32万亿)%.1f%%" % (MG["rzmre"][-1] / 23200 * 100),
)
print("20日变动 %.2f%% | 5日变动 %.2f%% | 情绪热度分 %.0f" % (chg20, chg5, score))

js = "const MG_EM = " + json.dumps(MG, ensure_ascii=False, separators=(",", ":")) + ";"
block = (
    "// ===MG_EM_START=== 真实全国两融余额序列(沪+深, akshare macro_china_market_margin_sh/sz, 单位:亿元) ===MG_EM_END===\n"
    + js
    + "\n"
)
