...
| **4.7** | Paper Trading Simulation（退出实验）| pending | 设计已定稿（`Phase4.6-4.8_Technical_Design.md`）：双轴隔离 6 组合（选择轴 S1/S2/S3 × 退出轴 E1/E2/E3），Phase 1 仅跑 S1+E1/E2/E3；退出框架 v1.0 四层（硬止损/移动止盈/评分衰减代理/时间退出，优先级 Strategy>Rating>Portfolio）；Alpha 指标（年化/夏普/Calmar/最大连亏） |
| **4.8** | Execution Intelligence Engine（占位）| pending | 设计占位（不开发）：入场合成层（集策略组合+标的当期+市场判断，非 follow 原始 `entry_rules`）+ 出场 + 仓位；`entry_mode` 预留；待 4.6/4.7 数据成熟后立项 |
